...
class DisputeResult:
    ...
    responsible_party: str = ""                   # "buyer" | "seller" | "courier" | "platform" | "unresolved"
    ruling_matched_rule: str = ""                 # 命中的平台裁决规则编号
...
请严格按以下 JSON 格式输出，不要包含任何额外文字：
{
  ...
  "compensation": {"type": "refund|resend|coupon|return_refund|combination|insurance_claim", "amount_yuan": 数字或0, "detail": "详细说明"},
...
# ═══════════════════════════════════════════════════════════════════════
# 平台裁决规则（注入 MediatorAgent prompt）
# ═══════════════════════════════════════════════════════════════════════
...
_MOCK_AFTERSALE_DB = {
    "ORDER_WM20240601_001": {
        ...
        "buyer_evidence": {
            ...
            "complaint_time": "2024-06-03 14:22:00",   # 签收后约2小时
        ...
        "seller_evidence": {
            "verification_video": {
                ...
                "duration_seconds": 58,
                "recorded_at": "2024-06-01 14:30:00",  # 发货前30分钟
...
async def _mock_get_after_sale_evidence(order_id: str | None = None) -> str:
    ...
    未接入真实售后系统时的降级方案。
    按 order_id 查询 _MOCK_AFTERSALE_DB，返回结构化的 JSON 字符串。
    """
    ...
    summary = {
        ...
        "seller_evidence": {
            ...
            "insured_amount_yuan": data["seller_evidence"]["shipping_insurance"]["insured_amount_yuan"],
    ...
    async def collect(
        ...
        **kwargs,
        ...
        if order_id:
            ...
            # 售后举证（双方证据 + 物流保价信息）
            tasks.append(("after_sale_evidence", _mock_get_after_sale_evidence(order_id)))
...
class DisputeCoordinator:
    ...
    async def resolve(
        ...
        *,
        ...
        # 从 mediator 的 evidence 中反查责任方和命中规则（先于 steps.append 提取）
        responsible_party = ""
        matched_rule = ""
    ...
    async def _run_mediator_agent(
        ...
        try:
            ...
            # 组装 evidence：责任占比 + 补偿方案 + 命中的规则 + 升级原因
            evidence_parts = []
            ...
            has_third_party = (
                ...
                or (resp and isinstance(resp, dict) and resp.get("courier_percent", 0) > 0)
            )
